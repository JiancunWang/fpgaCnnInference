...
#pragma HLS inline
    ...
    kl_result_t mulResult[SQR_KS_SIZE];
...
//									}
                        ...
                        if(ciDiv == chInPEDiv-1 ){
	                        ...
	                        #pragma HLS unroll
                                ...
                                hls::AXISetBitFields(outLStrmValue,int(ememCnt * DOT_BITS * 2), int(DOT_BITS *2) , kl_result_t(outData[ememCnt]));
								#ifndef __SYNTHESIS__
...
kl_result_t halfValueYolo  = 1<<(IMG_POS_IN + WEIGHTS_POS + SCALES_POS - IMG_POS_OUT-1);
    ...
    #pragma HLS LOOP_TRIPCOUNT MIN = 8 AVG = 16 MAX = 32
        ...
        #pragma HLS LOOP_TRIPCOUNT MIN = 8 AVG = 16 MAX = 32
            ...
            #pragma HLS pipeline II = 1
                kl_result_t raw[IEMEM_1ADOTS];
                kl_result_t bias[IEMEM_1ADOTS];
                kl_result_t mean[IEMEM_1ADOTS];
                kl_result_t variance[IEMEM_1ADOTS];
                kl_result_t batchData[IEMEM_1ADOTS];
                kl_result_t batchDataRelu[IEMEM_1ADOTS];
                kl_result_t batchDataDiv10Int[IEMEM_1ADOTS];
                ...
                //Normal rows process
                    ...
                    if( 0 == (coDiv16%4)){
                        ...
                        ExtMemToApFixSync<DOT_BITS * 2 ,outs_strm_t,kl_result_t,IEMEM_1ADOTS>(outResult,raw);
                    }
