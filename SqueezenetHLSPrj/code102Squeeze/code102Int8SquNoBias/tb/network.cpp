...
network_t *getNetWorkConfig(){
    ...
    ////  For 256PE Weights = 64co * 4ci * 512Depth * 9   = 256*512*9 = Dots
    //// Layer Attributes: (NAME  WIDTH HEIGHT CH_IN CH_OUT	KERNE PAD STRIDE RELU GPOOL	BN SUBL_NUM SUBL_SEQ DPI DPO WPO BPO
    addLayer(net, layer_t("conv1" , 261,  261,    3,    96,	   7,  0,     2,   1,	 2,	0,       1,	      0,  0, -3,  7,  7));
    addLayer(net, layer_t("f2s1"  ,  64,   64,   96,    16,	   1,  0,     1,   1,	 0,	0,       1,	      0, -3, -4,  7,  7));
    addLayer(net, layer_t("f2e1"  ,  64,   64,   16,    64,	   1,  0,     1,   1,	 0,	0,       2,	      0, -4, -4,  7,  7));
    addLayer(net, layer_t("f2e3"  ,  64,   64,   16,    64,	   3,  1,     1,   1,	 0,	0,       2,	      1, -4, -4,  7,  7));
    addLayer(net, layer_t("f3s1"  ,  64,   64,  128,    16,	   1,  0,     1,   1,	 0,	0,       1,	      0, -4, -4,  7,  7));
    addLayer(net, layer_t("f3e1"  ,  64,   64,   16,    64,	   1,  0,     1,   1,	 0,	0,       2,	      0, -4, -4,  7,  7));
    addLayer(net, layer_t("f3e3"  ,  64,   64,   16,    64,	   3,  1,     1,   1,	 0,	0,       2,	      1, -4, -4,  7,  7));
    addLayer(net, layer_t("f4s1"  ,  64,   64,  128,    32,	   1,  0,     1,   1,	 0,	0,       1,	      0, -4, -3,  7,  7));
    addLayer(net, layer_t("f4e1"  ,  64,   64,   32,   128,	   1,  0,     1,   1,	 2,	0,       2,	      0, -3, -3,  7,  7));
    addLayer(net, layer_t("f4e3"  ,  64,   64,   32,   128,	   3,  1,     1,   1,	 2,	0,       2,	      1, -3, -3,  8,  8));
    addLayer(net, layer_t("f5s1"  ,  32,   32,  256,    32,	   1,  0,     1,   1,	 0,	0,       1,	      0, -3, -4,  7,  7));
    addLayer(net, layer_t("f5e1"  ,  32,   32,   32,   128,	   1,  0,     1,   1,	 0,	0,       2,	      0, -4, -3,  7,  7));
    addLayer(net, layer_t("f5e3"  ,  32,   32,   32,   128,	   3,  1,     1,   1,	 0,	0,       2,	      1, -4, -3,  7,  7));
    addLayer(net, layer_t("f6s1"  ,  32,   32,  256,    48,	   1,  0,     1,   1,	 0,	0,       1,	      0, -3, -4,  7,  7));
    addLayer(net, layer_t("f6e1"  ,  32,   32,   48,   192,	   1,  0,     1,   1,	 0,	0,       2,	      0, -4, -3,  7,  7));
    addLayer(net, layer_t("f6e3"  ,  32,   32,   48,   192,	   3,  1,     1,   1,	 0,	0,       2,	      1, -4, -3,  8,  8));
    addLayer(net, layer_t("f7s1"  ,  32,   32,  384,    48,	   1,  0,     1,   1,	 0,	0,       1,	      0, -3, -3,  7,  7));
    addLayer(net, layer_t("f7e1"  ,  32,   32,   48,   192,	   1,  0,     1,   1,	 0,	0,       2,	      0, -3, -3,  7,  7));
    addLayer(net, layer_t("f7e3"  ,  32,   32,   48,   192,	   3,  1,     1,   1,	 0,	0,       2,	      1, -3, -3,  7,  7));
    addLayer(net, layer_t("f8s1"  ,  32,   32,  384,    64,	   1,  0,     1,   1,	 0,	0,       1,	      0, -3, -2,  6,  6));
    addLayer(net, layer_t("f8e1"  ,  32,   32,   64,   256,	   1,  0,     1,   1,	 2,	0,       2,	      0, -2, -2,  6,  6));
    addLayer(net, layer_t("f8e3"  ,  32,   32,   64,   256,	   3,  1,     1,   1,	 2,	0,       2,	      1, -2, -2,  7,  7));
    addLayer(net, layer_t("f9s1"  ,  16,   16,  512,    64,	   1,  0,     1,   1,	 0,	0,       1,	      0, -2, -3,  7,  7));
    addLayer(net, layer_t("f9e1"  ,  16,   16,   64,   256,	   1,  0,     1,   1,	 0,	0,       2,	      0, -3, -2,  7,  7));
    addLayer(net, layer_t("f9e3"  ,  16,   16,   64,   256,	   3,  1,     1,   1,	 0,	0,       2,	      1, -3, -2,  7,  7));
    addLayer(net, layer_t("conv10",  16,   16,  512,  1000,	   1,  0,     1,   1,	 0,	0,       1,	      0, -2, -3,  8,  8));
    return net;
}
//================================================================
...
// Set variables
    ...
    if( (layer.kernel == 3) && (layer.kernel == 1)){
       coPeAlignment = PE_KS_CO; 
    }else{
       coPeAlignment = PE_KL_CO;
    }
    chOutAlignment = CEIL_DIV(layer.channelsOut,coPeAlignment)*coPeAlignment;

    int widthOut=1+std::floor((float)(layer.width+2*layer.padSize-layer.kernel)/(poolSize*layer.stride));
    int heightOut=1+std::floor((float)(layer.height+2*layer.padSize-layer.kernel)/(poolSize*layer.stride));
...
// Set memAddrBias,scales,means,vars, and Weights , address Bytes
    ...
    printf("curImgOutAddr                =%16d\n", (int)curImgOutAddr);
    printf("curImgInAddr                 =%16d\n", (int)curImgInAddr);
    printf("||**********************************************||\n");
}
//================================================================
    ...
    for(int ci = 0; ci < chIn; ci++){
        for(int kernel = 0 ; kernel < kernelSquare ; kernel++){
            ...
            for(int co = 0; co < chOutAlignment; co++){
                ...
                //// conv1, weights, ci=2,co=6,k=26, should be 1, but fpga think is 2 , by cqiu
                if( 3 == chIn){
                	if( (ci == 2) && (6==co) && (26 == kernel))
                		dataTmp = 1;
                }
//                if( (6 == co) && (ci == 2))
