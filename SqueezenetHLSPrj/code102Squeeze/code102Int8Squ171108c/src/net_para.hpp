...
const int NUM_IMG_CACHE_LINES_KL = 1 << LOG2_NUM_IMG_CACHE_LINES;
const int LOG2_NUM_IMG_CACHE_LINES_KS = NBITS(KS_SIZE);
const int NUM_IMG_CACHE_LINES_KS = 1 << LOG2_NUM_IMG_CACHE_LINES_KS;
...
// Add extra data to prevent overflow, in fact,we should extra add NBITS(channels_in).
const int KL_ADD_WIDTH =  CEIL_DIV(NBITS(SQR_KL_SIZE),8)*8;//8
// (8bit x 2 + KL_ADD_LEN) , why 2, because result = 8bit x 8bit = 16bits
const int KL_RESULT_WIDTH = DOT_BITS * 2 + KL_ADD_WIDTH;//24bits
const int OUTL_STRM_WIDTH = KL_RESULT_WIDTH * PE_KL_CO;//24*8=192
// Add extra data to prevent overflow, in fact,we should extra add NBITS(channels_in).
const int KS_ADD_WIDTH =  NBITS(SQR_KS_SIZE)+5;//8
// (8bit x 2 + KS_ADD_WIDTH) , why 2, because result = 8bit x 8bit = 16bits
const int KS_RESULT_WIDTH = DOT_BITS * 2 + KS_ADD_WIDTH;//24bits
const int OUTS_STRM_WIDTH = KS_RESULT_WIDTH * PE_KS_CO;//24*64=1536bits
...
const int PE_KS_CO_PORT = PE_KS_CO/BRAM_1ADOTS;//16
...
const int PE_KS_CO_PORT_DIV4 = PE_KS_CO_PORT/WEMEM_1ADOTS_DIV4;//16/4
const int MAX_WIDTH_X_CHIN_DIV16_KL = MAX_DIMENSION*IEMEM_1ADOTS/(PE_KL_CI*IEMEM_1ADOTS);//1024x16 dots(for int8)
const int MAX_WIDTH_X_CHIN_DIV4_KL  = MAX_WIDTH_X_CHIN_DIV16_KL * BRAM_1ADOTS;
const int MAX_WIDTH_X_CHIN_DIV16_KS = 1024;//1024x16 dots(for int8)
const int MAX_WIDTH_X_CHIN_DIV4_KS = MAX_WIDTH_X_CHIN_DIV16_KS * BRAM_1ADOTS;//1024x16 dots(for int8)
...
const int MAX_WBUFF_BYTES_KS = DOT_BYTES * MAX_DOT(MAX_NUM_SPLIT_CHOUT * SQR_KS_SIZE , MAX_NUM_SPLIT_CHOUT * BIAS_NUM );
const int MAX_WBUFF_SIZE_KS = MAX_DOT(MAX_WBUFF_BYTES_KS / WEMEM_1ADOTS , BLOCK_SIZE);
//================================================================
//=Type-Definitions
//================================================================
//ap_int and ap_uint
...
typedef ap_uint<NBITS(WEMEM_1ADOTS)> bias_t;
...
typedef ap_int<KL_RESULT_WIDTH> kl_result_t;
typedef ap_int<KS_RESULT_WIDTH> ks_result_t;
